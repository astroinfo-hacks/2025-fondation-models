...
os.environ["HF_HUB_CACHE"] = (
    "/pbs/throng/training/astroinfo2025/model/euclid_desi/hf_home/hub"
)
os.environ["HF_DATASETS_CACHE"] = (
    "/pbs/throng/training/astroinfo2025/model/euclid_desi/hf_home/datasets"
)
...
class EuclidDESIDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.dataset = load_dataset(
            "msiudek/astroPT_euclid_desi_dataset", split=split, cache_dir=cache_dir
        )
        ...
        if sample["spectrum"] is not None:
            ...
            if flux is not None:
                spectrum_data = {
                    ...
                    "wavelength": (
                        torch.from_numpy(np.array(wavelength)).float()
...
def test_dataloader():
    ...
    try:
        ...
        print("DataLoader created with batch size 4")
        ...
        if batch["sed_fluxes"] is not None and len(batch["sed_fluxes"]) > 0:
            print(
                f"SED fluxes shape: {batch['sed_fluxes'][0].shape if batch['sed_fluxes'][0] is not None else 'None'}"
            )
        ...
        print(
            f"Individual bands available: VIS={has_vis}, Y={has_nisp_y}, J={has_nisp_j}, H={has_nisp_h}"
        )

        # Create visualization with RGB, spectrum, and individual bands
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle(
            f"Object {batch['object_id'][0]} (z={batch['redshift'][0]:.4f})",
            fontsize=16,
        )
        ...
        if (
            batch["spectrum"][0] is not None
            and batch["spectrum"][0]["flux"] is not None
        ):
            flux = batch["spectrum"][0]["flux"].numpy()
            wavelength = (
                batch["spectrum"][0]["wavelength"].numpy()
                if batch["spectrum"][0]["wavelength"] is not None
